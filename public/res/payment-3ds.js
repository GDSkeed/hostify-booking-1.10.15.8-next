...
var connectedAccount = typeof hfystripedata !== 'undefined' ? hfystripedata.connectedAccount : null;
var connectedAccountData = {stripeAccount: connectedAccount};
...
jQuery(document).ready(function($) {
    ...
    {
        var stripe = Stripe(stripeKey, connectedAccountData);
        ...
        var transactionId = document.querySelector("#transaction-id");
        var transactionMessage = document.querySelector("#transaction-message");
        var transactionDate = document.querySelector("#transaction-date");
        var transactionMethod = document.querySelector("#transaction-payment-method");
        var transactionAmount = document.querySelector("#transaction-amount");
        ...
        jQuery('#payment-form .pay-btn').on('click', function(){
        ...
        function addFormEventListener() {
            form.addEventListener('submit', function(e) {
                ...
                if (processFlow === 'reservation-payment') {
                    createReservation();
                } else {
                    createPaymentMethod();
                }
        ...
        function stripePaymentMethodHandler(stripeObject) {
            ...
            data.redirect_url = typeof hfystripedata !== 'undefined' ? hfystripedata.return_url : null;
            data.user_ip = '';
            data.user_agent = '';
            data.form_type = 'payment_card'; // for internal info
            ...
            if (!(result && result.success)) {
                ...
                if (result && result.requires_action) {
                    ...
                    try {
                        ...
                        transactionId.textContent = result.paymentData.id;
                        transactionDate.textContent = result.paymentData.created;
                        transactionMethod.textContent = result.paymentData.card;
                        transactionAmount.textContent = result.paymentData.amount;
