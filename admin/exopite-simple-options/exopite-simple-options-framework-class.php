...
/**
 * Last edit: 2019-05-27 // fixes by softrest.eu
 *
 * INFOS AND TODOS:
...
if ( ! class_exists( 'Exopite_Simple_Options_Framework' ) ) :
	...
	class Exopite_Simple_Options_Framework {
		...
		public function get_mo_file()
		{
			...
			$path   = HOSTIFYBOOKING_DIR . 'lang';
			$domain = 'hostifybooking';
			...
			if ( ! empty( $required_key_array ) && ! empty( $this->config ) ) {
				foreach ( $required_key_array as $key ) :
					if ( ! isset( $this->config[ $key ] ) ) {
						...
						$this->errors->add( "missing_config_key_{$key}", sprintf( esc_attr__( '%s is missing in the configuration array', 'hostifybooking' ), $key ) );
					}
				endforeach;
		...
		protected function set_properties()
		{
			...
			$this->dirname = wp_normalize_path( HOSTIFYBOOKING_DIR . 'admin/exopite-simple-options' );
		}
		...
		public function display_admin_error()
		{
			...
			if ( ! empty( $errors_array ) ) {
				...
				$message .= esc_attr__( 'Irks! An un-known error has occurred.', 'hostifybooking' );
			}
		...
		public function load_scripts_styles( $hook ) {
			...
			if ( $this->is_menu_page_loaded() || $this->is_metabox_enabled_post_type() ) :
				...
				$base = trailingslashit($url . 'assets');
		...
		public function display_options_page_header()
		{
			if (isset($_GET['msg'])) {
				// if ($_GET['msg'] == 'cleared') {
				// 	echo '<div id="message" class="updated fade"><p><svg style="vertical-align: top;height: 24px;margin-top: -4px;margin-right: 10px;" width="20" height="20" viewBox="0 0 73 75" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.4" d="M0.160156 34.8858C0.160156 33.9251 0.608394 33.0185 1.37423 32.4301L29.9578 8.51945C31.0871 7.65188 32.6652 7.65188 33.7945 8.51945L62.3781 32.4301C63.144 33.0185 63.5922 33.9251 63.5922 34.8858V71.0272C63.5922 72.7428 62.1898 74.1336 60.4597 74.1336H3.2926C1.5626 74.1336 0.160156 72.7428 0.160156 71.0272V34.8858Z" fill="#16A9FC"/><path d="M9.15234 27.017C9.15234 26.0563 9.60058 25.1497 10.3664 24.5613L38.95 0.650676C40.0793 -0.216891 41.6574 -0.216893 42.7867 0.650675L71.3703 24.5613C72.1361 25.1497 72.5844 26.0563 72.5844 27.017V63.1584C72.5844 64.874 71.1819 66.2648 69.4519 66.2648H12.2848C10.5548 66.2648 9.15234 64.874 9.15234 63.1584V27.017Z" fill="#167FFC"/><path fill-rule="evenodd" clip-rule="evenodd" d="M58.8177 28.9951L35.2706 52.5424L22.3555 40.0488L27.294 34.9436L35.188 42.5799L53.7951 23.9725L58.8177 28.9951Z" fill="white"/></svg> ' . __( 'Hostify cache cleared', 'hostifybooking' ) . '</p></div>';
				// }
				if ($_GET['msg'] == 'updated') {
					echo '<div id="message" class="updated fade"><p>' . __( 'Listings links updated', 'hostifybooking' ) . '</p></div>';
				}
			}

			echo '<form method="post" action="options.php" enctype="multipart/form-data" name="' . $this->unique . '" class="exopite-sof-form-js ' . $this->unique . '-form" data-save="' . esc_attr__( 'Saving...', 'hostifybooking' ) . '" data-saved="' . esc_attr__( 'Saved Successfully', 'hostifybooking' ) . '">';
			...
			// echo show_admin_tabs();

			echo '<header class="exopite-sof-header exopite-sof-header-js" style="min-height:20px">';
			// echo '<h1><svg style="vertical-align:text-bottom;height:24px" width="20" height="20" viewBox="0 0 73 75" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.4" d="M0.160156 34.8858C0.160156 33.9251 0.608394 33.0185 1.37423 32.4301L29.9578 8.51945C31.0871 7.65188 32.6652 7.65188 33.7945 8.51945L62.3781 32.4301C63.144 33.0185 63.5922 33.9251 63.5922 34.8858V71.0272C63.5922 72.7428 62.1898 74.1336 60.4597 74.1336H3.2926C1.5626 74.1336 0.160156 72.7428 0.160156 71.0272V34.8858Z" fill="#16A9FC"/><path d="M9.15234 27.017C9.15234 26.0563 9.60058 25.1497 10.3664 24.5613L38.95 0.650676C40.0793 -0.216891 41.6574 -0.216893 42.7867 0.650675L71.3703 24.5613C72.1361 25.1497 72.5844 26.0563 72.5844 27.017V63.1584C72.5844 64.874 71.1819 66.2648 69.4519 66.2648H12.2848C10.5548 66.2648 9.15234 64.874 9.15234 63.1584V27.017Z" fill="#167FFC"/><path fill-rule="evenodd" clip-rule="evenodd" d="M58.8177 28.9951L35.2706 52.5424L22.3555 40.0488L27.294 34.9436L35.188 42.5799L53.7951 23.9725L58.8177 28.9951Z" fill="white"/></svg> ' . __( 'Hostify Booking Engine', 'hostifybooking' ) . ' &nbsp; <small> <small><sup>ver. '.HOSTIFYBOOKING_VERSION.' </sup></small></small></h1>';

			// echo "<a href='" . get_admin_url() . "options-general.php?page=hostifybooking-plugin&updlistlink=1' style='margin-left:20px' class='button'>Refresh listings links</a>";

			echo '<span class="exopite-sof-search-wrapper"><input placeholder="'.esc_attr__('settings search...', 'hostifybooking').'" type="text" class="exopite-sof-search"></span>';

			echo '<fieldset>';
			echo '<span class="exopite-sof-ajax-message"></span>';
			// echo "<a href='" . get_admin_url() . "options-general.php?page=hostifybooking-plugin&clr=1' style='margin-left:20px' class='button hfy-clear-cache-btn'>Clear cache</a> &nbsp; ";
			submit_button( esc_attr__('Save Settings', 'hostifybooking'), 'primary ' . 'exopite-sof-submit-button-js', $this->unique . '-save', false, array() );
			echo '</fieldset>';
			echo '</header>';
		}
		...
		public function display_options_page_footer()
		{
			...
			submit_button( esc_attr__( 'Save Settings', 'hostifybooking' ), 'primary ' . 'exopite-sof-submit-button-js', '', false, array() );
			echo 	'</fieldset>';
			echo '</footer>';
			echo '</form>';
		}
		...
		public function get_menu_item( $section, $active = '', $force_hidden = false )
		{
			...
			echo '<li class="exopite-sof-nav-list-item' . $active . $hidden . '"' . $depend . ' data-section="' . $section_name . '" id="section_'.$section_name.'">';
		...
		public function display_page()
		{
			...
			if ( $this->config['type'] == 'menu' ) {
				add_action( 'exopite_sof_display_page_footer', array( $this, 'display_options_page_footer' ), 10, 1 );
