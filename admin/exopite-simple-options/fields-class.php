...
if (!class_exists('Exopite_Simple_Options_Framework_Fields')) {
	...
	abstract class Exopite_Simple_Options_Framework_Fields
	{
		...
		public function __construct($field = array(), $value = null, $unique = '', $config = array(), $multilang = false)
		{
			...
			$this->languages = ($this->multilang &&
				isset($this->multilang['languages']) &&
				is_array($this->multilang['languages'])) ? $this->multilang['languages'] : array($this->lang_default);
		}
		...
		*/
		...
		// 		$google_fonts_json_file = implode(DIRECTORY_SEPARATOR, array(HOSTIFYBOOKING_URL, 'admin', 'exopite-simple-options', 'assets', 'google-fonts.json'));
		// 		$response = wp_remote_get($google_fonts_json_file);
		// 		if (wp_remote_retrieve_response_code($response) == 200) {
		// 			$this->google_fonts = json_decode(wp_remote_retrieve_body($response));
		// 		}
		// 	}
