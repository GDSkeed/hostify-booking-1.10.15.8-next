	...
	class Exopite_Simple_Options_Framework_Field_typography extends Exopite_Simple_Options_Framework_Fields {
		...
		public function output() {
			...
			$default_variants = apply_filters(
				'exopite_sof_websafe_fonts_variants',
				array(
					'regular',
					'italic',
					'700',
			...
			if ( is_object( $google_json ) ) {
				...
				$is_google = isset( $googlefonts[ $family_value ] ) ? true : false;
				...
				echo '<optgroup label="' . esc_attr__( 'Web Safe Fonts', 'hostifybooking' ) . '">';
				...
				echo '<optgroup label="' . esc_attr__( 'Google Fonts', 'hostifybooking' ) . '">';
				...
				$self = new Exopite_Simple_Options_Framework(
					array(
				...
				if ( isset( $this->field['preview'] ) && $this->field['preview'] == true ) {
					...
					echo '<div class="exopite-sof-font-preview" style="font-size:20px">' . $preview_text . '</div>';
				}

				echo '<input type="text" name="' . $this->element_name( '[font]' ) . '" class="exopite-sof-typo-font hidden" data-atts="font" value="' . $value['font'] . '" />';

			} else {
				echo esc_attr__( 'Error! Can not load json file.', 'hostifybooking' );
			}
